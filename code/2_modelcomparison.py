...
# --- Sample data ----------------------------------------------------------------------------------------------------
df_tune = df.sample(n=min(df.shape[0], int(50e3))).query("target_iszero==0")
...
fit = GridSearchCV(ElasticNet(normalize=True, warm_start=True),
                   ...
                   n_jobs=4)\
    .fit(CreateSparseMatrix(metr=setdiff(features_lasso["metr"], exclude),
                            cate=setdiff(features_lasso["cate"], exclude),
...
fit = GridSearchCV(xgb.XGBRegressor() if TARGET_TYPE == "REGR" else xgb.XGBClassifier(),
                   ...
                   n_jobs=4) \
    .fit(CreateSparseMatrix(metr=setdiff(features_xgb["metr"], exclude),
                            cate=setdiff(features_xgb["cate"], exclude),
                            df_ref=df_tune).fit_transform(df_tune), df_tune["target"])
print(fit.best_params_)
...
fit = GridSearchCV(lgbm.LGBMRegressor() if TARGET_TYPE == "REGR" else lgbm.LGBMClassifier(),
                   ...
                   return_train_score=True,
                   n_jobs=4) \
    .fit(df_tune[setdiff(np.append(features_lgbm["metr"], features_lgbm["cate"]), exclude)], df_tune["target"],
         categorical_feature=[x for x in setdiff(features_lgbm["cate"], exclude).tolist() if "_ENCODED" in x]
         )
print(fit.best_params_)
df_fitres = pd.DataFrame.from_dict(fit.cv_results_)
