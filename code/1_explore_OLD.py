...
plot_distr(df, features=np.hstack(zip(metr, metr + "_BINNED")), target = "target_zscore",
...
print(varimp_cate)
varimp_cate_z = calc_imp(df, cate, "target_zscore", target_type=TARGET_TYPE)
print(varimp_cate_z)

# Check
plot_distr(df, cate,
           varimp=varimp_cate, target_type=TARGET_TYPE,
           color=color, ylim=ylim,
           ncol=3, nrow=2, w=18, h=12, pdf=plotloc + TARGET_TYPE + "_distr_cate.pdf")
plot_distr(df, cate, target = "target_zscore",
           varimp=varimp_cate_z, target_type=TARGET_TYPE,
           color=color,
